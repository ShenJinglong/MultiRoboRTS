...
/*
class PatrolActionNode: public ActionNode {
public:
  PatrolActionNode(const Blackboard::Ptr &blackboard_ptr):
      ActionNode::ActionNode("patrol action", blackboard_ptr) {
    ...
    patrol_behavior = new roborts_decision::PatrolBehavior(chassis_executor, blackboard, full_path);    
  }
...
class PatrolActionNode: public ActionNode {
public:
  PatrolActionNode(Blackboard::Ptr &blackboard_ptr, ChassisExecutor* &chassis_executor, const std::string &full_path):
      ActionNode::ActionNode("patrol action", blackboard_ptr) {
    blackboard_ = blackboard_ptr.get();
    patrol_behavior_ = new PatrolBehavior(chassis_executor, blackboard_, full_path);    
  }
  ~PatrolActionNode() {
    delete patrol_behavior_;
    delete blackboard_;
  }
...
class ChaseActionNode: public ActionNode {
public:
  ChaseActionNode(Blackboard::Ptr &blackboard_ptr, ChassisExecutor* &chassis_executor, const std::string &full_path):
      ActionNode::ActionNode("chase action", blackboard_ptr) {
    blackboard_ = blackboard_ptr.get();
    chase_behavior_ = new ChaseBehavior(chassis_executor, blackboard_, full_path);
  }
  ~ChaseActionNode() {
    delete chase_behavior_;
    delete blackboard_;
  }
private:
  void OnInitialize() {
